                ...
                {hide ? (
                    <View style={{position:'absolute', zIndex:999}}>
                        <TouchableOpacity onPress={()=>setHide(false)}>
                            <Text> <Text><Ionicons name="chevron-back-outline" color="white" size='25px'/></Text> </Text>
                        </TouchableOpacity>
                    </View>
                ) : (
                    <View style={{alignItems:'center', justifyContent:'space-between', flexDirection:'row', height:'70px', borderWidth:'3px', borderColor:'black', width:'400px', borderRadius:'20px', position:'absolute', opacity:'0.5', backgroundColor:'white', zIndex:999}}>
                    <TouchableOpacity onPress={()=>options != 0 ? setOptions(options-1) : 0} style={{paddingHorizontal:'20px'}}>
                        <Text> <Text><Ionicons name="chevron-back-outline" color="#444" size='25px'/></Text> </Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={{alignItems:'center'}} onPress={()=> handlePrint()}>
                        <Text style={{fontWeight:600, fontSize:'17px'}}>{presets[options].receiptsPerPage} per page</Text>
                        <Text style={{fontWeight:400, fontSize:'13px'}}>{presets[options].layout}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity onPress={()=>options != 6 ? setOptions(options+1) : 6} style={{paddingHorizontal:'20px'}}>
                        <Text> <Text><Ionicons name="chevron-forward-outline" color="#444" size='25px'/></Text> </Text>
                    </TouchableOpacity>
                    </View>
                )}
